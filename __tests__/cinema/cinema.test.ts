...
import { tokens } from './setupTestUsers';

describe('Cinema API', () => {
  const validCinema = Cinema.create({
    name: 'Salle 1',
  ...
  it('should reject invalid capacity', async () => {
    const res = await request(app)
      .post('/cinemas')
      .set('Authorization', `Bearer ${tokens.ADMIN}`)
      .send({ ...validCinema, capacity: 10 });

    expect(res.status).toBe(400);
  });

  it('should allow USER to view all cinemas', async () => {
    await Cinema.create({ ...validCinema }).save();
  ...
  it('should allow ADMIN to set a cinema in maintenance', async () => {
    const cinema = await Cinema.create({ ...validCinema }).save();
    const res = await request(app)
